...
namespace QuestSystem
{
    internal sealed class QuestPackManager : IDisposable
    {
        ...
        public bool TryGetStageNodeImmediate(string questTag, int nodeID, [NotNullWhen(true)] out StageNode? stageNode)
        {
            foreach(var pack in _packs)
            {
                stageNode = pack.GetNode(questTag, nodeID) as StageNode;
                
                if(stageNode != null) return true;
            }
            stageNode = null;
            return false;
        }
        
        public T? GetNodeImmediate<T>(string questTag, int nodeID) where T : NodeBase
        {
            foreach(var pack in _packs)
                if (pack.GetNode(questTag, nodeID) is T node) return node;
