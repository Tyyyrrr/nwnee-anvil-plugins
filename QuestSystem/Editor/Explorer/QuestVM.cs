...
namespace QuestEditor.Explorer
{
    public sealed class QuestVM : StatefulViewModelBase, ISelectable
    {
        ...
        public string TargetLevelString
        {
            get => Model.TargetLevel.ToString();
            set
            {
                if(int.TryParse(value, out var level) && Model.TargetLevel != level)
                {
                    var clampedLevel = Math.Clamp(level, 1, 40);

                    if(Model.TargetLevel != clampedLevel)
                        PushOperation(new SetQuestTargetLevelOperation(this, clampedLevel));
                }
                else RaisePropertyChanged(nameof(TargetLevelString));
            }
        }
        ...
        private sealed class SetQuestTargetLevelOperation(QuestVM vm, int level) : UndoableOperation(vm)
        {
            private readonly int _oldValue = vm.Model.TargetLevel;

            protected override void ProtectedDo()
            {
                vm.Model.TargetLevel = level;
                vm.RaisePropertyChanged(nameof(TargetLevelString));
            }
            ...
            protected override void ProtectedUndo()
            {
                vm.Model.TargetLevel = _oldValue;
                vm.RaisePropertyChanged(nameof(TargetLevelString));
            }
        }
