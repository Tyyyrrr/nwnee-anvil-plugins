...
namespace QuestSystem.Wrappers.Nodes
{
    internal sealed class RewardNodeWrapper : NodeWrapper<RewardNode>
    {
        ...
        private async Task<bool> GiveRewardAsync(NwPlayer player)
        {
            ...
            if(quest == null)
            {
                _log.Error($"{nameof(RewardNodeWrapper)} is not a part of any quest");    
                return false;
            }

            var targetLevel = quest.TargetLevel;
            ...
            if (node.Items.Count > 0)
            {
                ...
                foreach (var kvp in node.Items)
                {
                    ...
                    var item = await NwItem.Create(resRef, pc, kvp.Value <= 0 ? 1 : kvp.Value, string.IsNullOrEmpty(tag) ? string.Empty : tag);
            ...
            var xp = Math.Max(0,node.Xp);

            if(xp > 0)
            {
                xp = AdjustXPRewardForPCLevel(pc,targetLevel,xp);
                pc.Xp += Math.Max(0,xp);
                NWN.Core.NWScript.SetScriptParam("XP",xp.ToString());
                NWN.Core.NWScript.ExecuteScript("qs_updatexp",pc.ObjectId);
            }
        ...
        private static int AdjustXPRewardForPCLevel(NwCreature pc, int questTargetLevel, int xpReward)
        {
            var pcLevel = NWN.Core.NWScript.GetHitDice(pc.ObjectId);

            float mod = 0;

            if(pcLevel > questTargetLevel)
                mod = (pcLevel - questTargetLevel) * 0.1f; // TODO: use EasyConfig plugin to store modifier value

            int adjustedXp = xpReward - (int)(xpReward * mod);

            return Math.Max(adjustedXp,1);
        }
